...
function editTodo(id) {
  ...
  add_btn.removeEventListener("click", addNewTodo); 
  add_btn.addEventListener("click", () => {
    todo.task = task_input.value;
    todo.dueDate = date_input.value;
    todos.push(todo);
    saveToLocalStorage();
    task_input.value = "";
    date_input.value = "";
    add_btn.innerHTML = "<i class='bx bx-plus bx-sm'></i>";
    add_btn.removeEventListener("click", editTodo);
    add_btn.addEventListener("click", addNewTodo);
...
function toggleStatus(id) {
  ...
  if (todoIndex !== -1) {
    ...
    todos[todoIndex].status = todos[todoIndex].completed ? "completed" : "pending"; 
    saveToLocalStorage();
    showAllTodos();
  } else {
